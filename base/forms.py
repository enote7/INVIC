...
class EmailForm(forms.Form):
    email1 = forms.EmailField(label='Email 1', required=True, widget=forms.EmailInput(attrs={'placeholder': 'REQUIRED !'}))
    email2 = forms.EmailField(label='Email 2', required=False, widget=forms.EmailInput(attrs={'placeholder': 'Optional'}))
    email3 = forms.EmailField(label='Email 3', required=False, widget=forms.EmailInput(attrs={'placeholder': 'Optional'}))
    chat_name = forms.CharField(label='Chat Name', max_length=100, required=False, widget=forms.TextInput(attrs={'placeholder': 'Chat Name', 'class': 'hidden'}))
    conference_name = forms.CharField(label='Conference Name', max_length=100, required=False, widget=forms.TextInput(attrs={'placeholder': 'Conference Name', 'class': 'hidden'}))
    rules = forms.CharField(label='Rules', widget=forms.Textarea(attrs={'rows': 4, 'placeholder': 'Schedule Date & Time', 'class': 'hidden'}), required=False)
